...
const FrameworkPage: React.FC = () => {
  ...
  if (!content) {
    return (
      <div className="h-screen flex flex-col items-center justify-center p-20 text-center bg-ivory">
        <h1 className="serif text-7xl font-black mb-10 text-[#0a0f1a] italic tracking-tighter">Node Not Found.</h1>
        ...
        <Link to="/" className="px-16 py-8 bg-[#0a0f1a] text-white rounded-[2.5rem] font-black text-[12px] uppercase tracking-[0.6em] shadow-2xl hover:bg-indigo-600 transition-all duration-700">Return to Command</Link>
      </div>
    );
  }
  ...
  const renderContextualTool = () => {
      ...
      return (
        <div className="space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
             <input value={toolInput.v1} onChange={e => setToolInput({...toolInput, v1: e.target.value})} placeholder="Venture Concept (e.g. Neo-Banking)" className="bg-white/5 border border-white/10 rounded-2xl p-6 text-sm outline-none focus:border-emerald-400 transition-all shadow-inner" />
             <input value={toolInput.v2} onChange={e => setToolInput({...toolInput, v2: e.target.value})} placeholder="Industry Node (e.g. Fintech)" className="bg-white/5 border border-white/10 rounded-2xl p-6 text-sm outline-none focus:border-emerald-400 transition-all shadow-inner" />
          </div>
          <button onClick={handleToolExecute} disabled={aiLoading || !toolInput.v1 || !toolInput.v2} className="w-full py-6 bg-emerald-600 rounded-[2rem] font-black uppercase text-[11px] tracking-[0.5em] hover:bg-emerald-500 transition-all flex items-center justify-center shadow-2xl disabled:opacity-30">
    ...
    if (path === '/financials') {
      return (
        <div className="space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
             <input type="number" value={toolInput.v1} onChange={e => setToolInput({...toolInput, v1: e.target.value})} placeholder="EBITDA ($M)" className="bg-white/5 border border-white/10 rounded-2xl p-6 text-sm outline-none focus:border-emerald-400 transition-all shadow-inner" />
             <input type="number" value={toolInput.v2} onChange={e => setToolInput({...toolInput, v2: e.target.value})} placeholder="Total Asset Value ($M)" className="bg-white/5 border border-white/10 rounded-2xl p-6 text-sm outline-none focus:border-emerald-400 transition-all shadow-inner" />
          </div>
          <input value={toolInput.v3} onChange={e => setToolInput({...toolInput, v3: e.target.value})} placeholder="Primary Capital Goal (e.g. Expansion Debt)" className="w-full bg-white/5 border border-white/10 rounded-2xl p-6 text-sm outline-none focus:border-emerald-400 transition-all shadow-inner" />
          <button onClick={handleToolExecute} disabled={aiLoading || !toolInput.v1 || !toolInput.v2 || !toolInput.v3} className="w-full py-6 bg-emerald-700 rounded-[2rem] font-black uppercase text-[11px] tracking-[0.5em] hover:bg-emerald-600 transition-all flex items-center justify-center shadow-2xl disabled:opacity-30">
  ...
  return (
    <div className="bg-ivory min-h-screen relative">
          ...
          <div className="space-y-4">
            {tocItems.map(item => (
                ...
                className={`block text-[11px] font-black uppercase tracking-[0.4em] transition-all duration-700 text-left border-l-2 pl-6 py-2 ${
                    activeSection === item.id ? 'text-forest-green border-forest-green bg-emerald-50/20 translate-x-2' : 'text-slate-400 border-transparent hover:text-[#0a0f1a] hover:translate-x-1'
                }`}
                >
                {item.label}
                </button>
      ...
      {/* Breadcrumbs */}
      <nav className="px-10 lg:px-32 2xl:pl-[640px] pt-16 pb-8 flex items-center space-x-3 text-[11px] font-black uppercase tracking-[0.4em] text-slate-400">
        <Link to="/" className="hover:text-[#0a0f1a] transition-colors">Command</Link>
        <ChevronRight size={12} className="text-slate-200" />
        <span className="text-[#0a0f1a]">{content.title}</span>
      </nav>

      <div className="2xl:pl-[380px]">
        {/* Hero */}
        <section className="px-10 lg:px-32 py-32 border-b border-slate-50 bg-[#fdfdfc] relative overflow-hidden">
          <div className="absolute top-0 right-0 p-40 opacity-[0.015] pointer-events-none rotate-12 transform scale-150"><Landmark size={600} /></div>
          <div className="max-w-6xl">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
              <div className="flex items-center space-x-6 mb-12">
                <div className="w-16 h-[2px] bg-forest-green shadow-sm" />
                <span className="text-forest-green text-[12px] font-black uppercase tracking-[0.6em]">Sovereign Node Protocol</span>
              </div>
              <h1 className="serif text-8xl lg:text-[10rem] text-[#0a0f1a] font-black leading-[0.85] tracking-tighter mb-16 italic">
                {content.title}.
              </h1>
              ...
              <div className="flex flex-col sm:flex-row gap-10">
                <Link to="/schedule-meeting" className="px-16 py-8 bg-[#0a0f1a] text-white rounded-[2.5rem] font-black text-[12px] uppercase tracking-[0.6em] flex items-center justify-center group shadow-[0_32px_64px_-16px_rgba(10,15,26,0.3)]">
                  Schedule Meeting <ArrowRight className="ml-5 group-hover:translate-x-3 transition-transform" size={24} />
                </Link>
                <Link to="/contact" className="px-16 py-8 border-2 border-slate-100 text-[#0a0f1a] rounded-[2.5rem] font-black text-[12px] uppercase tracking-[0.6em] flex items-center justify-center hover:bg-white hover:border-[#0a0f1a]/10 hover:shadow-xl transition-all duration-700">
                  Contact Advisory
                </Link>
              </div>
            </motion.div>
        ...
        {isWorkSubPage && (
          <section id="ai-node" className="px-10 lg:px-32 py-32 bg-[#0a0f1a] text-white border-y border-white/5 scroll-mt-32 relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-24 relative z-10">
              <div className="lg:col-span-5 flex flex-col justify-center">
                <div className="flex items-center space-x-4 mb-10">
                  ...
                  <span className="text-[12px] font-black uppercase tracking-[0.5em] text-emerald-400">Contextual Node Analysis</span>
                </div>
              ...
              <div className="lg:col-span-7">
                <div className="h-full min-h-[600px] bg-white/5 rounded-[4rem] border border-white/10 p-16 flex flex-col relative group backdrop-blur-3xl shadow-[0_64px_128px_-32px_rgba(0,0,0,0.5)]">
                    ...
                    {aiLoading ? (
                      <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex flex-col items-center justify-center space-y-10">
                        <div className="relative w-24 h-24">
                            ...
                            <div className="absolute inset-0 border-[6px] border-emerald-400 border-t-transparent rounded-full animate-spin" />
                        </div>
                      ...
                      <motion.div key="result" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8 }} className="flex-1 flex flex-col">
                        <div className="flex justify-between items-center mb-10 border-b border-white/10 pb-10">
                          <p className="text-[12px] font-black uppercase tracking-[0.4em] text-emerald-400">Heuristic Analysis Artifact</p>
            ...
            <div>
              ...
              <div className="space-y-10">
                {content.deliverables.map((d, i) => (
                  <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} key={i} className="flex items-start p-10 bg-[#fdfdfc] rounded-[3.5rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-700 group">
                    <div className="w-14 h-14 bg-emerald-50 rounded-2xl flex items-center justify-center mr-8 flex-shrink-0 group-hover:bg-[#0a0f1a] transition-all duration-700 shadow-inner group-hover:shadow-2xl group-hover:scale-110">
                        <CheckCircle2 className="text-emerald-600 group-hover:text-emerald-400" size={28} />
                    </div>
                    <p className="text-[#0a0f1a] font-black text-2xl leading-tight tracking-tighter italic">{d}</p>
                  </motion.div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-12">
                {content.methodology.map((m, i) => (
                  <div key={i} className="flex items-start relative group">
                    <div className="w-16 h-16 bg-[#0a0f1a] text-white rounded-[1.8rem] flex items-center justify-center font-black text-xl flex-shrink-0 mr-10 shadow-[0_24px_48px_-12px_rgba(0,0,0,0.3)] group-hover:scale-110 transition-transform duration-700">
                      0{i + 1}
                    </div>
                    <div className="pt-2">
                      <p className="text-[#0a0f1a] font-black text-3xl mb-4 tracking-tighter italic">{m.split(':')[0]}</p>
        ...
        <section id="segments" className="px-10 lg:px-32 py-40 scroll-mt-48">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-[#0a0f1a] p-20 rounded-[4rem] text-white shadow-2xl relative overflow-hidden group">
              ...
              <h3 className="text-[12px] font-black uppercase tracking-[0.6em] mb-16 text-emerald-400">Target Capital Segments</h3>
              <ul className="space-y-10">
                {content.whoItsFor.map((s, i) => (
                    <li key={i} className="text-3xl font-black italic tracking-tighter border-l-4 border-emerald-500/20 pl-8 hover:border-emerald-500 transition-all duration-700 py-2">{s}</li>
                ))}
              </ul>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-forest-green p-20 rounded-[4rem] text-white shadow-2xl relative overflow-hidden group">
        ...
        {/* Case Study Example */}
        <section id="case-study" className="px-10 lg:px-32 py-40 bg-[#0a0f1a] text-white rounded-[5rem] mx-10 lg:mr-32 mb-40 relative overflow-hidden scroll-mt-48 shadow-[0_96px_192px_-48px_rgba(10,15,26,0.3)]">
          ...
          <div className="relative z-10 max-w-5xl">
            <div className="flex items-center space-x-6 mb-12">
              ...
              <span className="text-emerald-400 text-[12px] font-black uppercase tracking-[0.6em]">Engagement Artifact (Restricted Sample)</span>
            </div>
          ...
          <div className="max-w-5xl mx-auto space-y-16">
            {content.faq.map((f, i) => (
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} key={i} className="space-y-6 group">
                <h4 className="text-3xl font-black text-[#0a0f1a] tracking-tighter italic flex items-start leading-tight">
                  <div className="w-8 h-8 rounded-xl bg-slate-50 flex items-center justify-center mr-6 flex-shrink-0 group-hover:bg-[#0a0f1a] transition-all duration-700 shadow-inner group-hover:shadow-xl">
                    <AlertCircle className="text-forest-green group-hover:text-white" size={20} />
                  </div>
                  {f.q}
                </h4>
        ...
        {/* CTA BAND */}
        <section className="py-64 px-10 lg:px-32 bg-[#0a0f1a] text-white text-center relative overflow-hidden">
          ...
          <Link to="/schedule-meeting" className="inline-block px-24 py-10 bg-white text-[#0a0f1a] rounded-[3.5rem] font-black uppercase tracking-[0.6em] shadow-2xl hover:bg-emerald-400 hover:text-white transition-all duration-700 relative z-10 text-[13px] group">
